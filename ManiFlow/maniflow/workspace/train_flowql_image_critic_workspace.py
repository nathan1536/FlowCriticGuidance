"""
FlowQL with symmetric image critic.

Actor  : image encoding (MCR / DrQv2 CNN) → FlowMLP
Critic : same image encoding + action → Q  (no proprioceptive state)

Differences from train_flowql_adroit_workspace.py:
  - critic state_dim = model.obs_feature_dim  (not full_state_dim)
  - critic sees image encoding, not full_state
  - current obs encoding computed before critic update (eval mode, detached)
  - next obs encoding reused from target-Q block
  - compute_flowql_loss receives critic_states = image encoding
"""
...
class TrainFlowQLImageCriticWorkspace:
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # Image critic: state_dim = image embedding dim, not proprioceptive state
        state_dim  = self.model.obs_feature_dim
        action_dim = flowql_cfg.get("action_dim", 28)
        hidden_dim = flowql_cfg.get("critic_hidden_dim", 256)

        cprint(f"[ImageCritic] state_dim={state_dim} action_dim={action_dim} hidden_dim={hidden_dim}", 'cyan')
    ...
    def run(self):
        ...
        grad_norm     = flowql_cfg.get("grad_norm", -1)
        ...
        if lr_decay:
            ...
            cprint(f"Critic LR decay: T_max={lr_maxt}", 'green')
        ...
        if hasattr(dataset, 'has_rl_signals') and dataset.has_rl_signals and reward_tune != "no":
            ...
            cprint(f"Reward {reward_tune}: raw [{raw_min:.2f},{raw_max:.2f}] -> "
                   f"[{reward_data.min():.2f},{reward_data.max():.2f}]", 'green')
        ...
        if num_batches is not None:
            ...
            cprint(f"RandomSampler: {num_batches} batches/epoch", 'yellow')
        else:
        ...
        for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                for batch_idx, batch in tepoch:
                    ...
                    if ql_active and 'reward' in batch['obs']:
                        ...
                        with torch.no_grad():
                            ...
                            # ── Current obs encoding (online encoder, eval mode) ──
                            self.model.eval()
                            curr_nobs = self.model.normalizer.normalize(batch['obs'])
                            curr_nobs = dict_apply(curr_nobs,
                                lambda x: x[:, :self.model.n_obs_steps, ...].to(device))
                            curr_vis_cond = self.model.obs_encoder(curr_nobs)
                            curr_vis_cond = curr_vis_cond.reshape(batch_size, -1, self.model.obs_feature_dim)
                            states = curr_vis_cond.reshape(batch_size, -1)  # (B, obs_feature_dim)
                            self.model.train()

                            # ── Next obs encoding (ema encoder, eval mode) ──
                            next_obs_dict = {}
                            if getattr(self.model, 'is_embedding_policy', False):
                                next_obs_dict['img_embedding'] = torch.cat([
                                    batch['obs']['img_embedding'][:, 1:],
                                    batch['obs']['next_img_embedding'][:, :1],
                                ], dim=1)
                            elif getattr(self.model, 'is_image_policy', False):
                                next_obs_dict['image'] = torch.cat([
                                    batch['obs']['image'][:, 1:],
                                    batch['obs']['next_img'][:, :1],
                                ], dim=1)
                            else:
                                raise ValueError("ImageCritic workspace requires image or embedding policy")
                            ...
                            next_states = next_vis_cond.reshape(batch_size, -1)  # (B, obs_feature_dim)

                            # ── Sample next action ──
                            noise = torch.randn(batch_size, self.model.horizon,
                                                self.model.action_dim, device=device)
                            if max_q_backup:
                                n_repeat = 10
                                ...
                                noise_rpt = torch.randn(batch_size * n_repeat, self.model.horizon,
                                                        self.model.action_dim, device=device)
                                ...
                                next_actions_norm = ema_policy.sample_ode(
                                    x0=noise, N=num_sample_steps, vis_cond=next_vis_cond)[-1]
                                next_actions_raw = self.model.normalizer['action'].unnormalize(next_actions_norm)
                                next_a0 = next_actions_raw[:, 0].clamp(-1, 1)
                        ...
                        critic_grad_norm_val = torch.nn.utils.clip_grad_norm_(
                            self.critic.parameters(),
                            max_norm=grad_norm if grad_norm > 0 else float('inf')).item()
                    ...
                    ql_loss_val = 0.0
                    if ql_active:
                        # Encode current obs in train mode (with aug for DrQv2)
                        ...
                        # Always detach for image policies (encoder not updated by QL)
                        ql_vis_cond = vis_cond.detach()
                        # critic_states = image encoding (B, obs_feature_dim)
                        critic_states_for_ql = ql_vis_cond.reshape(batch_size, -1)

                        ql_loss, ql_log = self.model.compute_flowql_loss(
                            batch, ql_vis_cond, None, self.critic,
                            num_steps=num_sample_steps,
                            critic_states=critic_states_for_ql)
            ...
            if (self.epoch % cfg.training.sample_every) == 0:
                with torch.no_grad():
                    ...
                    mse = F.mse_loss(pred_action, batch['action'])
            ...
            cprint(f"[Epoch {self.epoch}] train={t_train_end-t_data_end:.1f}s "
...
def main(cfg):
    workspace = TrainFlowQLImageCriticWorkspace(cfg)
    workspace.run()
